...
vignette: >
  %\VignetteIndexEntry{Standardization of a GMRF}
...
$$\boldsymbol{A}^T\mathbf{u}=0 \text{ where }\boldsymbol{A}^T=E_X[D(X)]$$
...
$\widetilde{\mathbf{u}}$ will be also subject to $\boldsymbol{A}^T\widetilde{\mathbf{u}}=0$ if the effect is fixed. $\widetilde{\mathbf{D}}$ and $\widetilde{\mathbf{Q}}$ may or may not be equal to the original matrices, according to whether or not the effect already satisfied the requirements in terms of $\sigma^2$.
...
- `precision`: a matrix representing $\widetilde{\boldsymbol{Q}}$ (or $\widetilde{\boldsymbol{Q}}/C$ when `scale_Q=FALSE`)
- `basis`: a matrix representing $\widetilde{\boldsymbol{D}}(\boldsymbol{x})$. Equal to the input `D` unless $K=1$.
- `scaling_constant`: a positive number, representing the scaling constant $C$
- `mean_constraint`: a matrix with 1 column, representing $\boldsymbol{A}$. `NULL` for `fixed=FALSE`.
...
Once the GMRF is standardized using `standardize_GMRF()`, you can use the output directly in an **INLA** model to specify an effect. The key components of the output—`precision`, `basis`, and `mean_constraint`—are used to define a GMRF effect in INLA.

As an example, we consider a latent Gaussian model for a Normally-distributed response $Y$ where we only want to include an effect. Assuming the output of the `standardize_GMRF()` call for the effect is stored as `standardized_effect`, we first need to specify the basis matrix of the effect accessing the `basis` element of the list:

```{r, eval =FALSE}
# Create the data stack for INLA
data_stack <- inla.stack(
  data = list(Y = Y),       #response
  A = list(1, standardized_effect$basis),  # Use the standardized basis
  effects = list(
    data.frame(intercept = rep(1, length(Y))),
    list(u_rw1 = 1:10)  # Random walk coefficients
  )
)
```

Secondly, we need to include the effect in the formula of the INLA call. To specify a generic GMRF effect with null mean, we need to set `model = "generic0"`; then, the standardized version of the precision matrix can be specified using the argument `Cmatrix`; finally, remember to include the 0-mean constraint using the argument `mean_constraint` if the effect is considered fixed 
...
model <- inla(
  Y ~ -1 + intercept +  # No default intercept
    f(u_rw1, model = "generic0", constr = FALSE,
      Cmatrix = standardized_effect$precision,  # Standardized precision matrix
      extraconstr = list(
        A = t(standardized_effect$mean_constraint),  # Mean constraint (if fixed effect)
        e = matrix(rep(0, ncol(standardized_effect$mean_constraint)))  # Constraint RHS
      )
    ),
